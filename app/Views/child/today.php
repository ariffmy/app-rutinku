...
<header class="pt-2 mb-4">
    ...
    <p class="text-secondary mb-1"><?= esc($schedule['completed_count']) ?> daripada <?= esc($schedule['total_count']) ?> task selesai</p>
    <p class="fw-semibold text-primary mb-3">⭐ <?= esc($balance) ?> points · 🔥 <?= esc($currentStreak) ?> day streak</p>
    <div class="progress rounded-pill" role="progressbar" aria-label="Progress hari ini" aria-valuenow="<?= esc($schedule['completion_percentage']) ?>" aria-valuemin="0" aria-valuemax="100">
...
<?php if ($schedule['routines'] === [] || $schedule['task_count'] === 0): ?>
    <section class="card border-0 shadow-sm rounded-4">
        <div class="card-body p-4 text-center">
            ...
            <h2 class="h4">Tiada task hari ini</h2>
                        ...
                        <?php foreach ($routine['tasks'] as $task): ?>
                            <div class="list-group-item child-task-row <?= $task['is_completed'] ? 'task-completed' : '' ?> d-flex align-items-center gap-3 p-3">
                                <form action="<?= route_to($task['is_completed'] ? 'child.tasks.undo' : 'child.tasks.complete', $task['id']) ?>" method="post" class="flex-shrink-0">
                                    <?= csrf_field() ?>
                                    <button type="submit" class="task-toggle <?= $task['is_completed'] ? 'is-complete' : '' ?>" aria-label="<?= $task['is_completed'] ? 'Batalkan completion ' : 'Tandakan selesai ' ?><?= esc($task['title']) ?>">
                                        <?= $task['is_completed'] ? '✓' : '' ?>
                                    </button>
                                </form>
                                <div class="flex-grow-1">
                                    ...
                                    <div class="small text-secondary">
                                        ...
                                        <?php if (! $task['is_required']): ?> · Optional<?php endif ?>
                                    </div>
                                </div>
                                <div class="text-end">
