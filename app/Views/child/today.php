...
<div class="alert alert-success" role="status" data-perfect-day <?= $perfectDay === null ? 'hidden' : '' ?>>
    <strong>Perfect Day!</strong>
    <p class="mb-1">Semua rutin hari ini selesai.</p>
    <span>Bonus +<span data-perfect-day-bonus><?= esc($perfectDay['bonus_points'] ?? 0) ?></span> mata</span>
</div>
                ...
                <article class="col-12 col-md-6" data-task data-time="<?= esc($task['task_time'] ?? '') ?>" data-completed="<?= $task['is_completed'] ? '1' : '0' ?>" data-status="<?= esc($completionStatus) ?>" data-id="<?= esc($task['id']) ?>">
                    <div class="card child-task-card border-0 shadow-sm h-100"><div class="card-body">
                        <div class="child-task-details">
                            ...
                            <?php if (! empty($task['task_time'])): ?><div class="small text-secondary task-time" data-task-time><?= ui_icon('clock', 'Waktu:') ?><?= esc(ui_task_time($task)) ?></div><?php endif ?>
