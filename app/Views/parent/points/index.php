...
<header class="mb-4">
    <h1 class="h2 mb-1">Points</h1>
    <p class="text-secondary mb-0">Lihat ledger dan rekodkan adjustment untuk Child dalam family anda.</p>
</header>
...
<?php if ($children === []): ?>
    <div class="alert alert-info">Tiada Child aktif dalam family ini.</div>
<?php else: ?>
    <form method="get" action="<?= route_to('parent.points') ?>" class="card border-0 shadow-sm mb-4">
        <div class="card-body d-flex flex-column flex-md-row align-items-md-end gap-3">
            <div class="flex-grow-1">
                <label for="child-filter" class="form-label">Child</label>
            ...
            <button class="btn btn-outline-primary" type="submit">Lihat account</button>
        </div>
    </form>
    ...
    <div class="row g-4">
        <div class="col-12 col-lg-4">
            ...
            <form action="<?= route_to('parent.points.adjust') ?>" method="post" class="card border-0 shadow-sm">
                ...
                <div class="card-body p-4">
                    <h2 class="h4 mb-3">Manual adjustment</h2>
                    <div class="mb-3">
                    ...
                    <button class="btn btn-primary w-100" type="submit">Rekod adjustment</button>
                </div>
            </form>
        </div>
        ...
        <div class="col-12 col-lg-8">
            <section aria-labelledby="ledger-heading">
                <h2 id="ledger-heading" class="h4 mb-3">Ledger terkini</h2>
                <?php if ($account['history'] === []): ?>
                    <div class="card border-0 shadow-sm"><div class="card-body text-secondary">Belum ada transaksi points.</div></div>
                <?php else: ?>
                    <div class="card border-0 shadow-sm overflow-hidden">
                        <div class="table-responsive">
                            <table class="table align-middle mb-0">
                                <thead><tr><th>Tarikh</th><th>Jenis</th><th>Penerangan</th><th class="text-end">Points</th></tr></thead>
                                <tbody>
                                <?php foreach ($account['history'] as $transaction): ?>
                                    <tr>
                                        <td><?= esc($transaction['transaction_date']) ?></td>
                                        <td><span class="badge text-bg-light border"><?= esc($transaction['type']) ?></span></td>
