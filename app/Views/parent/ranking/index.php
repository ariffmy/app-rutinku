...
<div class="d-flex flex-wrap justify-content-between gap-2 mb-3"><h2 class="h4 mb-0"><?= esc(ui_label('period', $period)) ?> · <?= esc(ui_date($ranking['start_date'])) ?><?= $ranking['start_date'] !== $ranking['end_date'] ? ' hingga ' . esc(ui_date($ranking['end_date'])) : '' ?></h2><span class="text-secondary"><?= esc($ranking['total_completed_tasks']) ?> / <?= esc($ranking['total_scheduled_tasks']) ?> tugasan selesai</span></div>
<?php if ($ranking['rows'] === []): ?>
...
<?php else: ?>
    <div class="card border-0 shadow-sm overflow-hidden"><div class="table-responsive"><table class="table align-middle mb-0">
        ...
        <tbody><?php foreach ($ranking['rows'] as $row): ?><tr>
            <td class="h5">#<?= esc($row['rank']) ?></td><td class="fw-semibold"><?= esc($row['name']) ?></td><td><?= ui_icon('star') ?> <?= esc($row['earned_points']) ?></td><td><?= esc($row['completed_tasks']) ?> / <?= esc($row['scheduled_tasks']) ?></td><td><?= esc($row['completion_percentage']) ?>%</td><td><?= $period === 'daily' ? ($row['is_perfect_day'] ? ui_icon('check', 'Selesai') : '—') : esc($row['perfect_days']) ?></td><td><?= ui_icon('fire') ?> <?= esc($row['current_streak']) ?></td>
