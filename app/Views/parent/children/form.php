...
<header class="mb-4"><h1 class="h2 mb-1"><?= $editing ? 'Sunting Anak' : 'Tambah Anak' ?></h1><p class="text-secondary mb-0">Tetapkan e-mel dan kata laluan supaya Anak boleh log masuk ke dashboard sendiri.</p></header>
...
<form action="<?= $editing ? route_to('parent.children.update', $child->id) : route_to('parent.children.create') ?>" method="post" enctype="multipart/form-data" class="card border-0 shadow-sm"><div class="card-body">
    ...
    <div class="mb-3"><label for="email" class="form-label">E-mel Anak</label><input id="email" name="email" type="email" class="form-control" required maxlength="190" autocomplete="email" value="<?= esc(old('email') ?? ($child->email ?? '')) ?>"><div class="form-text">Anak menggunakan e-mel ini untuk log masuk.</div></div>
    <div class="mb-3"><label for="password" class="form-label"><?= $editing ? 'Kata laluan baharu (pilihan)' : 'Kata laluan' ?></label><div class="input-group"><input id="password" name="password" type="password" class="form-control" <?= $editing ? '' : 'required ' ?>minlength="8" maxlength="72" autocomplete="new-password"><button type="button" class="btn btn-outline-secondary" data-password-toggle aria-controls="password" aria-pressed="false" aria-label="Tunjukkan kata laluan"><i class="fa-solid fa-eye" data-password-toggle-icon aria-hidden="true"></i><span class="visually-hidden" data-password-toggle-label>Tunjukkan kata laluan</span></button></div><div class="form-text"><?= $editing ? 'Biarkan kosong untuk mengekalkan kata laluan semasa.' : 'Gunakan sekurang-kurangnya 8 aksara.' ?></div></div>
    <div class="mb-3"><label for="password_confirm" class="form-label">Sahkan kata laluan</label><div class="input-group"><input id="password_confirm" name="password_confirm" type="password" class="form-control" <?= $editing ? '' : 'required ' ?>minlength="8" maxlength="72" autocomplete="new-password"><button type="button" class="btn btn-outline-secondary" data-password-toggle aria-controls="password_confirm" aria-pressed="false" aria-label="Tunjukkan kata laluan"><i class="fa-solid fa-eye" data-password-toggle-icon aria-hidden="true"></i><span class="visually-hidden" data-password-toggle-label>Tunjukkan kata laluan</span></button></div></div>
